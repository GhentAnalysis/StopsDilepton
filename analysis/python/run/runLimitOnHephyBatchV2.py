...
for i, estimator in enumerate(signalEstimators):
    ...
    if options.subset and not int(mass[0])%5==0: continue
    #if int(mass[0])>250: continue
    ...
    if options.useTxt:
        estimator += " --useTxt"
    if options.combine:
        os.system(cmd+" 'python run_combination.py --signal %s --only=%s --version v9 --controlRegions fitAll'"%(options.signal, estimator))
    else:
        if year > 2016 and not options.signal=='TTbarDM':
            os.system(cmd+" 'python run_limit.py --signal %s --year %s --only=%s --version v9 --skipFitDiagnostics --unblind --genRecoNuis --fitAll'"%(options.signal, str(year), estimator))
        else:
